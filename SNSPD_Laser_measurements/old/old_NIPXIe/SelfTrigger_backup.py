...
# Global flags for debug and display
DEBUG = False
DISPLAY = False
...
def calculate_rise_fall_times(data, sample_rate=None):
    ...
    """
    # Find pulse baseline (mean of first few points) and extrema
    ...
    # Calculate threshold levels (10%, 90% from baseline to minimum)
    level_10 = baseline - 0.1 * (baseline - pulse_min)
    level_90 = baseline - 0.9 * (baseline - pulse_min)
    
    # Rising edge: from baseline to minimum (before idx_min)
    ...
    # Falling edge: from minimum back to baseline (after idx_min)
...
def Simple_pulse_analysis(data, trig, time, time_previous, event):
    # Calculate baseline from first 10 points
    pre_mean_val = np.mean(data[:10]) if len(data) >= 10 else data[0]
...
def SingleTDMS_analysis():
    ...
    with TdmsFile.open(in_filename) as tdms_file:
        ...
        for event in range(totalEvents-1):
            ...
            if DISPLAY: event_display_2ch(chSig, chTrig, 'Waveform')
            pulseCount += 1
...
if __name__ == "__main__":
    # Set global debug and display flags from arguments
