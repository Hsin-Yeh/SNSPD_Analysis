#!/usr/bin/env python3
"""
Fit all 50 blocks with:
- Tail cut: 75.7 ns (removes long tail)
- Plot range: up to 76.0 ns (shows full range after cut)
- Loose mean bounds: ±30 ps from block 10 reference
"""
...
T_MAX_NS = 79.0
cut_time_ns = 76.2
tail_cut_ns = 75.7  # NEW: More aggressive tail cut
plot_max_ns = 76.0  # NEW: Plot range for visualization

results_all = {}
block_ids = []

print("="*80)
print("FITTING ALL BLOCKS with 75.7 ns tail cut, 76.0 ns plot range")
print("="*80)
...
for block_idx in range(len(histograms)):
    ...
    A_guesses = []
    for mu_ref in FIXED_MEANS:
        idx_closest = np.argmin(np.abs(time_trimmed - mu_ref))
        A_guesses.append(max(hist_trimmed[idx_closest], 100))
    
    for i, mu_ref in enumerate(FIXED_MEANS):
        sigma_init = 0.05
        A_init = A_guesses[i]
        
        initial_params.extend([mu_ref, sigma_init, A_init])
        bounds_lower.extend([mu_ref - 0.030, 0.010, A_init * 0.2])
        bounds_upper.extend([mu_ref + 0.030, 0.200, A_init * 3.0])
    ...
    try:
        ...
        # Extract results
        fit_results = {}
        for i in range(len(FIXED_MEANS)):
            mu = popt[3*i]
            sigma = popt[3*i + 1]
            A = popt[3*i + 2]
            fit_results[i] = {
        ...
        print(f"Block {block_id:3d} @ {power_uw:7.4f} µW | σ: {sigmas[0]:6.2f}, {sigmas[1]:6.2f}, {sigmas[2]:6.2f}, {sigmas[3]:6.2f} ps | R²: {r_squared:.6f}")
...
output_dir = Path('/Users/ya/SNSPD_analyzed_output/TCSPC/SMSPD_3/power_sweep/74mV/peak_fits_75p7')
...
for block_id in sorted(results_all.keys()):
    ...
    plot_end_ns = 76.0
    plot_end_bin = np.searchsorted(time_trimmed, plot_end_ns)
    if plot_end_bin == 0:
    ...
    ax2.set_xlim([time_plot[0], 76.0])
    
    plt.tight_layout()
    output_file = output_dir / f'block_{block_id:03d}_fit_75p7.png'
...
print("\n" + "="*100)
print("PEAK WIDTH SUMMARY (σ in ps)")
print("="*100)
...
for block_id in sorted(results_all.keys()):
    ...
    print(f"{block_id:5d} | {data['power']:9.4f} | {sigmas[0]:9.2f} | {sigmas[1]:9.2f} | {sigmas[2]:9.2f} | {sigmas[3]:9.2f} | {data['r_squared']:.6f}")
