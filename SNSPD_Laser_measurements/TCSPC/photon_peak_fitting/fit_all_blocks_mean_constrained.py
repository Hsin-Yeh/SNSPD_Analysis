#!/usr/bin/env python3
"""
Refit all blocks with light constraints:
- Fix mean positions to be close to block 145 reference (±50 ps tolerance)
- Allow widths to vary freely with power
- This prevents photon peak position drift while adapting peak widths to power
"""
...
# Block 145 reference parameters (from previous unconstrained fit with 75.7 ns cut)
BLOCK_145_MUS = np.array([75.2356, 75.3386, 75.4688, 75.5880])  # ns
MU_TOLERANCE_NS = 0.050  # ±50 ps tolerance on mean positions

print("="*80)
print("REFITTING ALL BLOCKS with Mean Position Constraints (Block 145 Reference)")
print("="*80)
print(f"\nBlock 145 Reference Mean Positions (2.62 µW):")
for i, mu in enumerate(BLOCK_145_MUS):
    n_label = 4 - i
    print(f"  n={n_label}: μ={mu:.4f} ns")
print(f"\nConstraint: Mean positions fixed within ±{MU_TOLERANCE_NS*1000:.0f} ps of reference")
print(f"            Peak widths free to vary with power\n")
print("="*80 + "\n")

# Load powers and other info
...
for block_idx in range(len(histograms)):
    ...
    # Initial parameters (use loose bounds as before)
    initial_params = []
    bounds_lower = []
    bounds_upper = []
    ...
    for i, (mu_ref, A_guess) in enumerate(zip(BLOCK_145_MUS, A_guesses)):
        sigma_init = 0.050  # Start with ~50 ps width
        
        initial_params.extend([mu_ref, sigma_init, A_guess])
        
        # Tight mean bounds, loose sigma bounds
        bounds_lower.extend([mu_ref - MU_TOLERANCE_NS, 0.010, A_guess * 0.2])
        bounds_upper.extend([mu_ref + MU_TOLERANCE_NS, 0.200, A_guess * 3.0])
...
print("\n" + "="*80)
print(f"Successfully fitted {len(results_all)} blocks with mean position constraints")
print("="*80)

# Generate plots for each block
output_dir = Path('/Users/ya/SNSPD_analyzed_output/TCSPC/SMSPD_3/power_sweep/74mV/peak_fits_mean_constrained')
...
for block_id in sorted(results_all.keys()):
    ...
    # Mark tail cut and mean constraints
    ax1.axvline(75.7, color='orange', linestyle=':', linewidth=2, alpha=0.7, label='Tail cut (75.7 ns)')
    for mu_ref in BLOCK_145_MUS:
        ax1.axvline(mu_ref, color='gray', linestyle=':', linewidth=1, alpha=0.3)
    
    ax1.set_xlabel('TOA (ns)', fontsize=12, weight='bold')
    ax1.set_ylabel('Counts', fontsize=12, weight='bold')
    ax1.set_title(f'Block {block_id} @ {power_uw:.4f} µW | R² = {r_squared:.4f} | Mean Position Constrained', 
    ...
    plt.tight_layout()
    output_file = output_dir / f'block_{block_id:03d}_mean_constrained.png'
...
summary_file = output_dir / 'fit_results_mean_constrained.json'
...
print("\n" + "="*100)
print("PEAK WIDTH SUMMARY - MEAN POSITION CONSTRAINED (Block 145 Reference)")
print("="*100)
...
print("="*100)
print("\n✓ All fits complete with mean position constraints!")
